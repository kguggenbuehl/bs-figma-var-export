...
async function run(includeDarkMode) {
...
function tokenNamePrimitives(name) {
    return '--' + name.replace(/\//g, "-").replace('Typography-', "");
}
function tokenNameSemantic(name) {
...
async function resolveAlias(value) {
    ...
    if (typeof value === "object" && value !== null && "r" in value) {
        return colorToCssHSL(value);
    }
    return String(value);
}
// build CSS for primitive values
async function buildPrimitiveSection(collection, primitivesFilterValues) {
    let css = `    /* Primitives */\n`;
    return (async () => {
...
async function buildSemanticSection(collection, semanticFilterValues, includeDarkMode) {
